class Host::ListingsController < ApplicationController
  ...
  def edit
    @listing = currrnt_user.listings.find(params[:id])
  end
...
if @listing.update(listing_update_params)
  ...
  def listing_create_params
   params.require(:listing).permit(:title, :about, :max_guests, :address_line1, :address_line2, :city, :state, :postal_code, :country)
  end
